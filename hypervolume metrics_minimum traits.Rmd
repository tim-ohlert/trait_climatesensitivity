...
```{r}
core_trait_z.1 <- subset(core_trait_z.1, avg.height_A.z != "NA" & sla.z != "NA" & rdmc.z!= "NA" & seed_mass.z!= "NA" )
...
```{r}
hv_split <- base::split(core_trait_z.1[,c("avg.height_A.z", "sla.z", "rdmc.z", "seed_mass.z")], core_trait_z.1$rep_year)
```
...
```
 ...
 #hv_func <- function(x) {
 #  hypervolume_gaussian(data = x, name = x$rep_year)
 #}
...
```{r}
hv_volumes <- parLapply(cl,  hv_split, hypervolume_gaussian)
...
hv_div <- data.frame(rep_year, alpha, evenness, dispersion, originality)
...
```{r}
write.csv(hv_climate, "hv_climate-11.21.20.csv")
```
